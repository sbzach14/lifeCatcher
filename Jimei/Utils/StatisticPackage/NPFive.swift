...
class NPFiveGame{
    ...
    static func calWinners(diyDealStatus:[[Bool]], diyDealNum:[Int], deck: [Card], args: [Int], rankRules: [Int], suitRules: [Int]) -> ([GameReturnPlayerInfo],[Int]) {
        ...
        for i in 0..<playerNum {
            (allPlayCards[i].evaluateFlag,allPlayCards[i].cardType, allPlayCards[i].isPair) = NPFiveGameHandEvaluator(
                rankRules: rankRules,
                suitRules: suitRules
        ...
        if leftCards.count < NPFiveGame.getMinCardNum(playerNum: playerNum,handNum: handNum, communityNum: communityNum, dealType: dealType, diyDealNum: diyDealNum, diyDealStatus: diyDealStatus){
            leftCards = []
        }
...
class NPFiveGameHandEvaluator{
    var rankRules: [Int]
    var suitRules: [Int]
    var ruleDict: [Int: ([NPFiveCard]) -> (Int, String, Int)] = [:]
    ...
    func evalHand(cards: [Card], redJokerValueRange: Int, blackJokerRange: Int, KValueRange: Int, QValueRange: Int, JValueRange: Int, samePointComparision: Int, isPairSameRank: Int, pairRequirement: Int)->(Int, String, Int){
        ...
        let num1 = NPFiveCard(card: cards[0], redJokerValueRange: redJokerValueRange, blackJokerValueRange: blackJokerRange, KValueRange: KValueRange, QValueRange: QValueRange, JValueRange: JValueRange)
        
        let num2 = NPFiveCard(card: cards[1], redJokerValueRange: redJokerValueRange, blackJokerValueRange: blackJokerRange, KValueRange: KValueRange, QValueRange: QValueRange, JValueRange: JValueRange)
    ...
    func eval_black8PlusRedJoker(cards: [NPFiveCard]) -> (Int, String, Int) {
    ...
    func eval_black8PlusBlackJoker(cards: [NPFiveCard]) -> (Int, String, Int){
    ...
    func eval_red8PlusRedJoker(cards: [NPFiveCard]) -> (Int, String, Int) {
    ...
    func eval_red8PlusBlackJoker(cards: [NPFiveCard]) -> (Int, String, Int){
    ...
    func eval_kingpair(_ cards: [NPFiveCard]) -> (Int, String, Int) {
        var rank = 0
    ...
    func eval_ninepointfive(_ cards: [NPFiveCard]) -> (Int, String, Int) {
        var rank = 0
    ...
    func eval_onepair(_ cards: [NPFiveCard]) -> (Int, String, Int) {
        let rank = 0
        var cardType: String = ""
        if self.pairRequirement == 0 {
            if cards[0].rank == cards[1].rank {
                cardType = "对" + ClassifierSettingArgs.CardNumberReportDic[cards[0].rank]!
    ...
    func eval_holecard(cards: [NPFiveCard]) -> (Int, String, Int) {
        var rank = 0
    ...
    func blackRedJudger(card: NPFiveCard) -> Int{
        //黑色
    ...
    class NPFiveCard{
        ...
        init(card: Card, redJokerValueRange: Int, blackJokerValueRange: Int, KValueRange: Int, QValueRange: Int, JValueRange: Int){
            let rule = ClassifierSettingArgs.targetSetting[6] as! NPFiveGameRule
            //Rank Initialization
            self.rank = card.rank
            //suit Initialization
