...
class FCGame{
    ...
    static func calWinners(diyDealStatus:[[Bool]], diyDealNum:[Int], deck: [Card], args: [Int], rankRules: [Int], suitRules: [Int]) -> ([GameReturnPlayerInfo],[Int]) {
        ...
        for i in 0..<playerNum {
            (allPlayCards[i].evaluateFlag,allPlayCards[i].cardType, allPlayCards[i].isPair) = FCGameHandEvaluator(
                rankRules: rankRules,
                suitRules: suitRules
        ...
        if leftCards.count < FCGame.getMinCardNum(playerNum: playerNum,handNum: handNum, communityNum: communityNum, dealType: dealType, diyDealNum: diyDealNum, diyDealStatus: diyDealStatus){
            leftCards = []
        }
...
class FCGameHandEvaluator{
    var rankRules: [Int]
    var suitRules: [Int]
    var ruleDict: [Int: ([FCCard]) -> (Int, String, Int)] = [:]
    ...
    func evalHand(cards: [Card],redJokerValueRange: Int, blackJokerValueRange: Int, KValueRange: Int, QValueRange:Int, JValueRange:Int, pointComparision: Int, samePointComparision: Int, cardRankRule: Int)->(Int, String, Int){
        ...
        let num1 = FCCard(card: cards[0], redJokerValueRange: redJokerValueRange, blackJokerValueRange: blackJokerValueRange, KValueRange: KValueRange, QValueRange: QValueRange, JValueRange: JValueRange, cardRankRule: cardRankRule)
        
        let num2 = FCCard(card: cards[1], redJokerValueRange: redJokerValueRange, blackJokerValueRange: blackJokerValueRange, KValueRange: KValueRange, QValueRange: QValueRange, JValueRange: JValueRange,cardRankRule: cardRankRule)
    ...
    func eval_isPair(cards:[FCCard]) -> (Int, String, Int){
    ...
    func eval_isHighCard(cards: [FCCard]) -> (Int, String, Int) {
        let cardType: String = "高牌" + ClassifierSettingArgs.CardNumberReportDic[cards[0].originalRank]!
        return (cards[0].rank, cardType, 0)
    }
    
    func eval_isPoint(cards:[FCCard]) -> (Int, String, Int) {
    ...
    class FCCard{
        ...
        init(card: Card, redJokerValueRange: Int, blackJokerValueRange: Int, KValueRange: Int, QValueRange:Int, JValueRange:Int, cardRankRule: Int){
            let rule = ClassifierSettingArgs.targetSetting[11] as! FCGameRule
