...
接收端连接页和会话工具栏均提供独立功能设置：可启停提示音/结果播报，选择扬声器或耳机，调整本地音量与语速，并设置视频/黑屏交互、时间模式和黑屏亮度。设置保存到接收端专用 UserDefaults 键，只影响手机2本机，不改写识别端方案或桌面转发配置。接收会话不提供结果页或常驻显示模式切换按钮；与原识别页一致，屏幕显示设为“无”时双击无动作，启用黑屏模式后从实时画面双击进入黑屏，配置时间模式时黑屏内显示日期和时间，再次双击返回实时画面。黑屏/时间伪装期间隐藏连接状态、导航栏、设置与断开入口，仅保留全屏黑色或时间内容。
