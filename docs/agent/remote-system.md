...
- 识别-接收端：同一识别页面以本机序列号分别连接 source 和 receiver 角色；继续识别并发送结果，同时播放桌面端转发给 receiver 的提示音、结果或文字。不额外订阅本机视频，不显示牌序结果页；识别设置的屏幕显示仍提供相机图像，接收声音设置独立保存。它占用该序列号唯一的 receiver 槽，因此不能与另一台纯接收端同时连接到同一序列号。
