...
审计基线：2026-08-12。范围仅为“幻影”核心识别链、手机2接收端和远程旁路；普通 `cls_main` 采集链未接入远程功能。

## 已关闭的逻辑风险

- 角色入口先分识别端/接收端；手机2不创建 `CurrentVisionObjectRecognitionViewModel`、Core ML 或相机会话。
- 远程版识别端固定创建 bridge 并自动连接；网络和用户所选的720p/1080p、30/60 FPS缩放均为现有识别输出的异步旁路，不参与 ROI、检测、牌序恢复或规则计算。手机1固定跳过音频会话、提示音播放器和 TTS，本机及其耳机均不发声。
- 三角色可任意顺序进入等待房间；手机1显示服务器/手机2/桌面端，手机2显示服务器/手机1/桌面端的在线、重连中、离线状态，手机1的状态变化只显示静默 toast。
...
- 用真实 iPhone 分别验证前后摄像头方向、镜像、720p/1080p与30/60 FPS组合的持续帧率、识别高帧率同时编码时的温度/电量/内存与本地识别准确率。
- 当前大陆固定 EIP 需要做 Wi-Fi、蜂窝、VPN、网络切换和20秒租约内外的端到端验证；新加坡仍未配置。绑定域名后补 TLS/TURN-TLS 验证并移除明文直连放宽。
- 验证手机2各 iOS 中文声音下男女声意图、语速、重复次数和蓝牙 A2DP 路由；同时验证手机1在扬声器、蓝牙和有线输出下始终无提示音/TTS。不同系统音色本来不要求一致。
